...
m_indexCount(0),
m_tracking(false),
m_deviceResources(deviceResources), totalradiansX(0), totalradiansY(0)
{
...
void PseudoSphereRenderer::CreateWindowSizeDependentResources()
{
	...
	static const XMVECTORF32 at = { 10, 0, 10, 0 }; //{ -12.0f, /*12.0f*/0.0f, 0.0f, 0.0f };
...
{
	auto Sphere = ref new PseudoSphere();
...
void PseudoSphereRenderer::TrackingUpdate(float* positionX)
{
	if (m_tracking)
	{
		float radians = XM_2PI * 2.0f * positionX[0] / m_deviceResources->GetOutputSize().Width;
		totalradiansY += XM_2PI * 2.0f * positionX[1] / m_deviceResources->GetOutputSize().Width;
		Rotate(radians);
		totalradiansX += radians;
	}
}
